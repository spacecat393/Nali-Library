...
public class Box
{
	...
	public static void draw(MemoS rs, float[] v_float_array, float[] c_float_array, int array_buffer, int ap_size, int t_first, int t_count)
	{
		OpenGlHelper.glBindBuffer(OpenGlHelper.GL_ARRAY_BUFFER, array_buffer);
		GL20.glVertexAttribPointer(rs.attriblocation_int_array[0], ap_size, GL11.GL_FLOAT, false, 0, 0);
		...
		GL11.glDrawArrays(GL11.GL_TRIANGLES, t_first, t_count);
	}
}
